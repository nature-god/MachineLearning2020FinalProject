# from https://github.com/sjruan/tptk/tree/4c399e1789a399c516904b5b5af23a4896b58064
# the viterbi algorithm for time-inhomogeneous Markov processes
...
class ViterbiAlgorithm:
    ...
    def initialize_state_probabilities(self, observation, candidates, initial_log_probabilities):
        ...
        for candidate in candidates:
            if candidate['pos'] not in initial_log_probabilities:
                raise Exception('No initial probability for {}'.format(candidate['pos']))
            log_probability = initial_log_probabilities[candidate['pos']]
            initial_message[candidate['pos']] = log_probability
        ...
        for candidate in candidates:
            self.last_extended_states[candidate['pos']] = ExtendedState(candidate['pos'], None, observation['pos'], None)
        self.prev_candidates = [candidate['pos'] for candidate in candidates]
        ...
        for cur_state in cur_candidates:
            ...
            for prev_state in prev_candidates:
                if isinstance(prev_state, tuple):
                    ...
                    prev_state = prev_state['pos']
                log_probability = message[prev_state] + self.transition_log_probability(prev_state, cur_state['pos'],
            ...
            result.new_message[cur_state['pos']] = max_log_probability + emission_log_probabilities[cur_state['pos']]
            ...
            if max_prev_state is not None:
                transition = (max_prev_state, cur_state['pos'])
                if transition_descriptors is not None:
                    transition_descriptor = transition_descriptors[transition]
                else:
                    transition_descriptor = None
                extended_state = ExtendedState(cur_state['pos'], self.last_extended_states[max_prev_state], observation['pos'],
                                               transition_descriptor)
                result.new_extended_states[cur_state['pos']] = extended_state
        return result
